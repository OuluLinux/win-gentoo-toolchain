...
multilib_src_compile() {
	...
	multilib_src_install() {
		...
		if [[ ${CHOST} == *-mingw* ]]; then
			...
			shopt -s nullglob
			dll_candidates=( "${ED}"/usr/bin/liblzma-*.dll )
			shopt -u nullglob
			[[ ${#dll_candidates[@]} -gt 0 ]] && have_shlib=1
		else
			[[ -f "${ED}/usr/$(get_libdir)/liblzma$(get_libname)" ]] && have_shlib=1
		fi

		if ! tc-is-static-only && (( ! have_shlib )); then
